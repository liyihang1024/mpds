...
usage:运行脚本，依次输入待分割文件->分割关键词->输出文件名(可省略)
...
def fileParse():
    ...
    for eachLine in fp:
        ...
        kw = re.search(keyWord, eachLine)                               # search()会扫描整个字符串查询关键字，并返回第一个成功的匹配
        if kw is not None:                                              # 如果字符串中匹配到关键字，search()方法将返回 None
            number.append(lineNumber)                                   # 将关键字的行号记录在number中
            dataEntry.append(eachLine)
    ...
    for i in range(0, size - 1):
        ...
        fp_w = open(outFileName + dataEntry[i] + '.cif', 'w')            # 将截取出的内容保存为 .cif 文件
        for key in destLines:
            fp_w.write(key)
        fp_w.close()
    # with open('entry.txt','w+') as entry:
    #     entry.write(dataEntry)
    df = pd.DataFrame(dataEntry)
    df.to_excel('spinel0.xlsx')
